...
Eigen::Matrix3d ForwardKinematics::omegaHat(
  ...
  hatOmega << 0, -omega.z(), omega.y(), omega.z(), 0, -omega.x(),
      -omega.y(),  // cppcheck-suppress constStatement
      omega.x(), 0;
  return hatOmega;
}
...
Eigen::VectorXd ForwardKinematics::getRevoluteScrewAxis(
  ...
  Eigen::VectorXd screwAxis(
      spatial_vector_size);  // cppcheck-suppress constStatement
